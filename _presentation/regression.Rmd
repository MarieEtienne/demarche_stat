...
n <- 20
imax <-9
x <- round(rnorm(n, mean=0, sd = 4),1)
y <- rnorm(n, mean= beta0+beta1*x - beta2 * x^2, sd = 10)
...
* La variance de $Y_{new}$ prend en compte l'incerertitude sur les paramètre et la variabilité naturelle des observations ($\sigma^2$)
...
### Prédiction d'une nouvelle valeur $Y_{new}$ pour un $x$ donné
...
$$Y_k = \hat{beta_0} +\hat{beta_1} x_{k,1}+ \ldots + \hat{beta_p} x_{k,p}$$
