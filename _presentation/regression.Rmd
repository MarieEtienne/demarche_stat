...
template: parametre
###  Estimateur de $\theta$ : $\class{alea}{T}$

$$\class{alea}{T} = \begin{pmatrix} B_0 \\ B_1 \\ \vdots \\ B_p \end{pmatrix} = (X^\intercal X )^{-1} X^\intercal Y$$
...
$$\class{alea}{T}  \sim \mathcal{N}_{p+1}\left(\theta, \sigma^2 (X^\intercal X )^{-1}\right).$$

--

### Estimation de $\theta$ : $\class{fixe}{\hat{\theta}}$
...
$$RSS = || Y- X \, \class{alea}{T} ||^2 = \sum_{k=1}^{n} (Y_k - B_0 + B_1 x_1 + \ldots + B_p x_p)^2$$
...
$$\class{fixe}{RSS_{obs}} = || Y- X \, \class{fixe}{\hat{\theta}} ||^2 = \sum_{k=1}^{n} (Y_k - \class{fixe}{\hat{\beta}_0} + _0_1 x_1 + \ldots + \class{fixe}{\hat{\beta}_p} x_p)^2$$
...
$\class{alea}{B_0} + \class{alea}{B_1} x \sim\mathcal{N}(\beta_0 + \beta_1 x, \sigma^2 \class{clair}{Constante} )$
...
$$c \class{alea}{T} \sim\mathcal{N}(c\theta , \sigma^2 \class{clair}{Constante_c} )$$
