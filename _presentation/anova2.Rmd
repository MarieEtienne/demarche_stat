...
* Quelle est la fréquence moyennes des femmes pratiquant la pétanque ?
...
$\alpha_3$ est donc l'effet différentiel de l'actiité 3 (la pétanque) par rapport à l'activité de référence 1 (la natation) pour le sexe 1 (les femmes)..
...
---
template: model
## Version contrainte  du modèle du modèle Mcomp
...
Y_{IJn_{IJ}}\end{pmatrix},
 \quad
{\bf{X}} =\overset{\color{gray}{\begin{matrix}\mu & \alpha_{2} &  \ldots & \alpha_{I} & $\beta_2 & \ldots &\beta_{J} & \gamma_{22} & \gamma{23} & \ldots  &\gamma_{IJ}\end{matrix}}}{\begin{pmatrix}
...
$$\class{alea}{T} = \begin{pmatrix} \class{alea}{M} \\ \class{alea}{A_2} \\ \vdots  \\  \class{alea}{A_I} \\ B_2 \\ \vdots\\ \class{alea}{B_J} \\ \class{alea}{G_{22}} \\ \vdots \\ \class{alea}{G_{IJ}}\\ \end{pmatrix} = (X^\intercal X )^{-1} X^\intercal \class{alea}{Y}$$
...
Il faut faire un choix pour les parties confondues
...
template: comp_moyenne
## Moyennes ajustées pour le Activite
