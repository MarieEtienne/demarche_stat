...
## Etude de la féquance cardiaque au repos

On a mesuré la fréquence cardiaque de 20 femmes et 20 hommes ayant des niveaux de pratique sportive différents.
...
En plus du sexe, il est possible que le niveau de pratique sportive influence également la fréquence cardiaque

```{r sport, eval = TRUE, message=FALSE}
freqdata %>% mutate(Sport_Fact = as.factor(Sport)) -> freqdata
freqdata %>% ggplot() + aes(y = freqC, x = Sport_Fact, fill = Sexe) + geom_boxplot() + xlab('Sport') + ylab("Fréquence cardiaque")
```
--
...
.pull-left[
* Y a t il un effet du sexe ou du sport sur la fréquence cardiaque ? 

* Y a t il un effet du sexe  sur la fréquence cardiaque ? 
]

.pull-right[
* Y a t il un effet du sport sur la fréquence cardiaque ? 

* L'effet du sport sur la fréquence cardiaque est-il le même pour les hommes et les femmes ? 
]
---
template: intro
...
- et deux facteurs (variables qualitatives) pouvant prendre respectivement $I$ modalités (I=2 pour le Sexe)  et J modalités (J=5 pour le Sport)
...
fake_dta <- tibble( fact1 = rep(rep(1:I, each=J), effectif),
  ...
  mutate(Sexe = as.character(fact1), 
         Sport = as.character(fact2), 
         ord=0)  
...
ggplot(data=fake_dta) + 
  ...
  labs(x = 'Fréquence cardiaque', y = 'Pratique Sportive') +
