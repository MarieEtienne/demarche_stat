...
On a mesuré la fréquence cardiaque de 12 femmes et 12 hommes ayantactivités de loisirs différentes.
...
freqdata %>% mutate(Activite_Fact = as.factor(Activite)) -> freqdata
freqdata %>% ggplot() + aes(y = freqC, x = Activite_Fact, fill = Sexe, col = Se) + geom_boxplot(alpha = 0.5) + xlab('Activite') + ylab("Fréquence cardiaque")
```
...
Mcomp <- lm(freqC ~ Activite + Sexe + Sexe:Activite, data = freqdata)
