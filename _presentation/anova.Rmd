...
- $\class{rouge}{\mu_1}$ la fréquence cardiaque moyenne des femmes et $\class{rouge}{\mu_2}$ celle des hommes. 
- $\class{rouge}{\sigma^2}$ la variance commune à tous les groupes.
...
- $\class{rouge}{\alpha_1}$, l'effet différentiel d'être une femme par rapport à la différence et $\class{rouge}{\alpha_2}$ l'effet différentiel d'être un homme.
- $\class{rouge}{\sigma^2}$ la variance commune à tous les groupes.
...
- $I=2$ et  la convention $i=1$ pour les femmes et $i=2$ pour les hommes.
- $k=1, \ldots, n_i$, avec $n_1=20$ et $n_2=20$
- $\class{rouge}{\mu}$ la fréquence cardiaque moyenne des femmes, 
- $\class{rouge}{\alpha_1}$, l'effet différentiel d'être une femme par rapport à la différence et $\class{rouge}{\alpha_2}$ l'effet différentiel d'être un homme.
- $\class{rouge}{\sigma^2}$ la variance commune à tous les groupes.
...
<br><br><br><br>
<p style="color:#B40F20;font-size:35px;text-align:center;">Prenons une petite pause TD!!!</p> 
...
- de variance $\mathbb{V}(M_i) =  \frac{\class{rouge}{\sigma^2}}{n_i}$
--
...
- $\mu$ représente le comportement moyen du groupe 1 (groupe de référence) : $\class{fixe}{\hat{\mu}} =  \class{fixe}{y_{1\bullet}}$
--
...
En fait, $\hat{\mu}_i$ est choisi pour minimiser $RSS_{M_1, obs}$, la somme observée des erreurs au carré  donnée par  

$$RSS_{M_1, obs} = \sum_{i=1}^I \sum_{k=1}^{n_i} (y_{ik} - \hat{\mu}_i)^2.$$ 
--
...
$RSS_{M_1,obs}$ se réécrit dans un cadre général comme 

$$RSS_{M_1, obs} = \sum_{i=1}^I \sum_{k=1}^{n_i} (y_{ik} - \hat{y}_{ik})^2.$$ 

$$RSS_{M_1} = \sum_{i=1}^I \sum_{k=1}^{n_i} (Y_{ik} - \hat{Y}_{ik})^2.$$ 
